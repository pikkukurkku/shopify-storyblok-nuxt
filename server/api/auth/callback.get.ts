...
export default defineEventHandler(async (event) => {
  ...
  const storedNonce = getCookie(event, 'shopify_oauth_nonce')
  ...
  try {
    ...
    try {
      ...
      throw createError({ statusCode: 400, statusMessage: 'OAuth token exchange failed' })
    }
    ...
    if (tokens.id_token && storedNonce) {
      ...
      if (payload?.nonce !== storedNonce) {
        throw createError({ statusCode: 400, statusMessage: 'Nonce mismatch' })
      }
    }
